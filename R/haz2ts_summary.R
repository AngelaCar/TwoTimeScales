...
#'
#' @param x an object of class 'haz2tsPGAM' returned by the function [fitpgam()]
#' @param ... further arguments
#'
#' @return a printed summary of the fitted model
#'
...
#' @export
#'
haz2tsPGAM_summary <- function(x,...){
  if (!inherits(x, "haz2tsPGAM")) stop("'x' must be a 'haz2tsPGAM' object")
...
#'
#' @param x an object of class 'haz2tsVCM' returned by the function [fitvcm()]
#' @param ... further arguments
#'
#' @return a printed summary of the fitted model
#'
...
#' @export
#'
haz2tsVCM_summary <- function(x,...){
  if (!inherits(x, "haz2tsVCM")) stop("'x' must be a 'haz2tsVCM' object")
  ...
  res <- list(nevents=nevents,ns=ns,cs=cs,AIC=AIC,BIC=BIC,ED=ED,
              logsmpar=logsmpar,smpar=smpar)
  if(!is.null(x$optimal_model$beta)){
    coeftab <- cbind(coef= hr$beta, secoef=hr$SE_beta,
                     expcoef=hr$HR,
                     lowerci=hr$HR - 1.96*hr$SE_HR,
  ...
  cat("  cs = ", res$cs, "\n")
  cat("\nOptimal smoothing: \n")
  cat("  log10(theta) = ", res$logsmpar[1], "\n")
  cat("  log10(phi) = ", res$logsmpar[2], "\n")
  cat("  theta = ", res$smpar[1], "\n")
  cat("  phi = ", res$smpar[2], "\n")
  cat("\n")
  if(is.null(res$coeftab)) cat("Model with no covariates") else {
    colnames(res$coeftab) <- c("beta", "se(beta)", "exp(beta)", "lower .95", "upper.95")
