#' Plot the one time scale smooth hazard
#'
#' @description  Plot the estimated smooth (log-) hazard with one time scale.
#'
#' @param fitted_model The output of the function `fit1ts`.
...
#'   (default), `"covariates"`, `"SE"`, `"slices"` or `"3dhazard"`.
...
plot_haz1ts <- function(fitted_model,
  ...
  if(which_plot == "hazard"){
    hazard_SE <- get_hazard_1d(fitted_model, plot_grid)
