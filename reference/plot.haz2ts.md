...
`plot.haz2ts()` is the plot method for objects of class `haz2ts`. It
produces several kinds of plots of the fitted model with two time scales
(see
[`fit2ts()`](https://angelacar.github.io/TwoTimeScales/reference/fit2ts.md)),
either in the original (t,s) plane, while respecting the constraint
imposed by the relation of the two time scales, or in the transformed
(u,s) plane.
