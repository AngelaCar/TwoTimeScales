...
It produces plots of the fitted model with two time scales (see
[`fit2ts()`](https://angelacar.github.io/TwoTimeScales/reference/fit2ts.md)),
fitted via LMMsolver. The two-dimensional plots are limited to the
transformed plane, that is only plots over `u` and `s` axes are
produced.
...
- x:
  ...
  The output of the function `fit2ts`. This is an object of class
  `"haz2tsLMM"`.
...
- which_plot:
  ...
  The type of plot required. Can be one of `"hazard"` (default),
  `"covariates"`, `"SE"`, `"slices"`, `"survival"` or `"cumhaz"` (see
  details section).
...
- plot_options:
  ...
  - `col_palette` A function defining the color palette. The default
    ...
    as a vector. We provide an example on how to create a function from
    any color palette below.
