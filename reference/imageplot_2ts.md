...
`imageplot_2ts()` plots an image of the two time scales hazard (or
survival or cumulative hazard) with contour lines. This is the default
call implemented in plot.haz2ts and other functions.
...
- z:
  ...
  dimensions compatible with those of `x` and `y`. The default is to
  plot the hazard.
...
- plot_options:
  ...
  - `log10hazard` A Boolean. Default is `FALSE`. If `TRUE`, then a
    log10-hazard surface is plotted.
