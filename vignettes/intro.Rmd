...
Estimation is performed using $P$-splines (univariate or bivariate) and Poisson regression.
...
This function works in the same way for the case of data with one time scale only, and for data over two time scales.
...
```{r 1ts-model, dependson = "1ts-data-prep", cache=TRUE}
...
Estimation over the half-plane $(u,s)$ is performed in the same way.
...
```{r 2ts-mod, dependson = "2ts-data-prep", cache=TRUE}
