...
@Carollo:2024 discusses the issue of extrapolated hazard beyond the maximum value observed for the time scale $t$. We can limit the plot of the hazard to those areas where it is not extrapolated. We do so by changing the values of `cut_extrapolated = T` and `tmax` in `plot_options`.
...
```{r survival, fig = T, chache = T, dependson="load-mod", fig.width=7, fig.align='center', fig.height=4.5}
...
```{r cumhaz, fig = T, chache = T, dependson="load-mod", fig.width=7, fig.align='center', fig.height=4.5}
